...
function addresponseevent(x)
{
    ...
    // Try to fetch the responses, fallback to local if CORS or fetch fails
    fetch('responses.json')
        .then(response => response.json())
        .then(data => {
            let resstring = data.responses.hasOwnProperty(x) ? data.responses[x] : data.responses[""];
            ...
            console.log("JSON response fetched successfully.");
        })
        .catch(() => {
            console.log("Failed to fetch responses.json, falling back to local data.");
            let resstring = responses.responses.hasOwnProperty(x) ? responses.responses[x] : responses.responses[""];
            const resnode = document.createTextNode(resstring); // Create a text node with the input value
            respara.classList.add("ourResponse"); // Set the class name for styling
            respara.appendChild(resnode); // Append the text node to the paragraph element
            element.appendChild(respara); // Append the paragraph element to the "conversation" element
